...
func init() {
	orm.RegisterModel(new(GxSlide))
}

// AddGxSlide insert a new GxSlide into database and returns
...
// GetGxSlideById retrieves GxSlide by Id. Returns error if
// Id doesn't exist
...
// GetAllGxSlide retrieves all GxSlide matches certain condition. Returns empty list if
// no records exist
func GetAllGxSlide(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(GxSlide))
	// query k=v
	for k, v := range query {
...
// UpdateGxSlide updates GxSlide by Id and returns error if
...
// DeleteGxSlide deletes GxSlide by Id and returns error if
...
func DeleteGxSlide(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&GxSlide{Id: id}); err == nil {
