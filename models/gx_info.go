...
func init() {
	orm.RegisterModel(new(GxInfo))
}

// AddGxInfo insert a new GxInfo into database and returns
...
// GetGxInfoById retrieves GxInfo by Id. Returns error if
// Id doesn't exist
...
// GetAllGxInfo retrieves all GxInfo matches certain condition. Returns empty list if
// no records exist
func GetAllGxInfo(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(GxInfo))
	// query k=v
	for k, v := range query {
...
// UpdateGxInfo updates GxInfo by Id and returns error if
...
// DeleteGxInfo deletes GxInfo by Id and returns error if
...
func DeleteGxInfo(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&GxInfo{Id: id}); err == nil {
